...
def data_lookup(tmp_upc, tmp_sc, tmp_vc, tmp_ic, tmp_csize, tmp_ncost, tmp_upc_cand2, tmp_vat, dict_error_flag, dict_vendor, dict_vend, param_etc=None):
    # print('#2')
            ...
            try:
                for row in ws_t.rows:
                    ...
                    if int(valid_date[0]) < 1 or int(valid_date[0]) > 12 or int(valid_date[1]) < 1 or int(valid_date[1]) > 31 or len(valid_date[2]) != 4:
                        ...
                        f_error_log.close()
                        break
                    if tmp_sc is None or len(tmp_sc) != 3 or int(tmp_sc) > len(dict_store_code) or int(tmp_sc) < 1:
                        f_error_log = open(file_error_log, 'a')
                        flag_error = 1
