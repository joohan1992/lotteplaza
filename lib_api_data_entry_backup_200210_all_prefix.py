            ...
            if idx3 == 1:
                ...
                if dict_link[u_key][24] == 1:
                    ...
                    result_old_case_cost, result_new_case_cost, result_price, result_old_margin, result_new_margin, result_srp = \
                        getSrpSet(dict_link[u_key][17], dict_link[u_key][25], arr2[-1], dict_link[u_key][27], dict_link[u_key][23])
                    dict_link[u_key][25] = result_old_case_cost
                    dict_link[u_key][26] = result_new_case_cost
                    dict_link[u_key][27] = result_price
                    dict_link[u_key][28] = result_old_margin
                    dict_link[u_key][29] = result_new_margin
                    dict_link[u_key][30] = result_srp
                else:
                    arr2.append(u_key2)
            ...
            try:
                for row in ws_t.rows:
                    ...
                    result_upc, result_ic, result_desc, result_csize, _, _, result_flag_processed_ce, result_upc_check, \
                        ...
                        result_price, result_old_margin, result_new_margin, result_srp, tmp_upc_cand2, detail_cat = \
                    ...
                    for col in row:
                        if idx2 == 0:
                            ...
                            if (tmp_ic != result_ic and result_ic not in getFormalItemCode(tmp_ic, tmp_vc, dict_vend)) or int(result_upc_check) == 0:
                                arr1.append(tmp_ic)
                                if tmp_ic != '':
                                    arr1.append(1)
                                else:
        ...
        if isfile(file_targ):
            ...
            for row in ws_t.rows:
                ...
                if row[16].value is None:
                    tmp_ic = ''
                if row[37].value is not None and row[37].value != "":
                    tmp_ic = row[37].value
                result_ic = tmp_ic
                ...
                try:
                    ...
                    arr1[14] = tmp_vc
                    arr1[16] = result_ic
                    if tmp_ic != result_ic and result_ic not in getFormalItemCode(tmp_ic, tmp_vc, dict_vend):
                        if tmp_ic != '':
                            arr1[17] = 1
                        else:
                            arr1[17] = 0
                    ...
                    if key_item in dict_item:
                        ...
                        list_item[dict_item[key_item]['idx']][28] += 10
                        ...
                        if result_srp is not None and result_srp != '' and result_srp != ' ':
                            ...
                            if dict_item[key_item]['key_srp_non'] > -1:
                                try:
                                    ...
                                    print(dict_item)
                                    print(key_item)
                                    print(dict_item[key_item]['key_srp_non'])
                                    print(list_non_srp_store_date[dict_item[key_item]['key_srp_non']])
